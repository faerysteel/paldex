...
fn compress(compression: Compression, data: &[u8]) -> Result<Vec<u8>> {
    ...
    let compressed = match compression {
        ...
        // Read-only vendoring (see entry.rs): oozextract has no encoder, so writing
        // Oodle-compressed paks — never exercised by paldex, which only reads the
        // game's existing pak — stays unsupported.
