        ...
        match self.compression_slot.and_then(|c| compression[c as usize]) {
            ...
            Some(comp) => {
                ...
                match comp {
                    ...
                    // Vendored from trumank/repak (MIT/Apache-2.0) and patched here: the
                    // upstream Oodle path loads the game's native oo2core DLL via
                    // `oodle_loader`, which only exists on Windows and doesn't help on
                    // Apple Silicon. `oozextract` is a pure-Rust, clean-room Kraken/
                    // Mermaid/Selkie/Leviathan decoder with no such constraint — see
                    // `paldex-sav`, which already verified it against real Palworld
                    // save files.
                    Compression::Oodle => {
                        let mut decompressed = vec![0; self.uncompressed as usize];
                        for (decomp_chunk, comp_range) in
                            decompressed.chunks_mut(chunk_size).zip(ranges)
                        {
                            oozextract::Extractor::new()
                                .read_from_slice(&data[comp_range], decomp_chunk)
                                .map_err(|_| Error::DecompressionFailed(Compression::Oodle))?;
                        }
                        buf.write_all(&decompressed)?;
                    }
                }
